...
dredit.View.prototype.CreateNavs = function() {
	$('#nav-pane').append(
			$('<ul></ul>').attr('id', 'editor-navs').attr('class',
					'nav nav-pills'));
}
...
dredit.View.prototype.CreateEditor = function() {
	$('#editor-pane').append(
			$('<div></div>').attr('id', 'editor-holder').append(
					$('<div></div>').attr('class', 'editor').attr('id',
							this.editor_id)));
...
dredit.View.prototype.CreatePicker = function() {
	...
	picker = new google.picker.PickerBuilder().setAppId(dredit.CLIENT_ID)
			.addView(view).setCallback($.proxy(this.PickerSelected, this))
			.build();
...
dredit.Page.prototype.CreateLabel = function() {
	...
	this.label_text = $('<a>Loading</a>').attr('href', '#' + this.tab_id)
...
dredit.TitleChanger.prototype.CreateField = function(field, label) {
	return $('<div class="control-group"></div>').append(
			$('<label class="control-label"></label').text(label)).append(
			$('<div class="controls"></div>').append(field));
}
...
dredit.TitleChanger.prototype.CreateUi = function() {
	...
	this.modal = $('<div class="modal"></div>').append(
			...
			.append(
					$('<div class="modal-body"></div>').append(
							$('<form class="form-horizontal">').append(
									this.CreateField(this.title, 'Title'))
									.append(
											this.CreateField(this.description,
													'Description')))).append(
					$('<div class="modal-footer"></div>').append(this.saver))
